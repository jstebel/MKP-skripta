...
která splňuje
\eq{\label{eq:weak_poisson_1d}
  \forall v\in V:\quad \int_0^L k(x)u'(x)v'(x) \d x = \int_0^L f(x)v(x) \d x - gv(0).
}
\end{enumerate}


Všimněme si dvou vlastností slabého řešení:
\begin{itemize}
  \item $u''$ nemusí existovat,
  \item $u'$ nemusí být spojitá, stačí existence integrálu na levé straně \eqref{eq:weak_poisson_1d}.
\end{itemize}
...
Nyní zvolíme konečně rozměrný vektorový podprostor $V_h\subset V$.
V tomto prostoru definujeme bázi $\{\varphi_j(x)\}_{j=1}^N$.
Tedy pro každou funkci $v_h\in V_h$ existuje právě jeden vektor $\vc v_h=(v_{h,1},...,v_{h,N})$
takový že 
\[
  v_h(x) = \sum\limits_{j=1}^N v_{h,j} \varphi_j(x).
\]
% 
Uvažujme nyní aproximativní řešení $u_h\in V_h$ ve tvaru
\[
  u_h(x) = \sum\limits_{j=1}^N ´u_j \varphi_j.
\]
% 
Hledáme tedy $\vc u$ (vektor  koeficientů lineární kombinace), splňující \eqref{eq:weak_poisson_1d} s testovacími funkcemi $v_h=\varphi_i$, $i=1,...,N$:
\eq{
  \int_0^L k(x) \left( \sum\limits_{j=1}^N u_j \varphi_j(x) \right)' \varphi_i'(x) \d x
...
pak \eqref{eqn:discrete_weak_form} lze zapsat jako soustavu lineárních rovnic pro neznámý vektor $\vc u$:

\begin{equation}
  \mat A \vc u = \vc b. \label{eqn:linear_system}
\end{equation}
