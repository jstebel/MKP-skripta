...
(hranice tvořena rovnými úseky). Výpočetní síť je rozdělení oblasti trojúhelníky {\it elementy} 
$T \in \elements$ tak, aby dva trojúhelníky sousedili buďto celou společnou hranou nebo 
společným vrcholem. Množina $\elements$ obsahuje všechny elementy sítě.
...
a na každém elementu $T_e$ jsou polynomy prvního stupně, tedy lineární funkce, viz. Obr. \ref{fig:base_2d_lin}. 
...
\subsection{Assemblace lineárního problému}
...
Pro rovnici \eqref{eq:weak_poisson} rozdělíme integrály přes celou oblast $\Omega$ na součet 
integrálů přes jednotlivé elementy $T$ a jejich okraje na hranici oblasti, což je průnik hranice 
elementu s hranicí oblasti: $\prtl T \cap \prtl \Omega$. Pro vnitřní elementy je druhý integrál nulový.
...
\[
l(v) = \sum_{T\in \elements} \left( \int_{T} f v \, \mathrm{d}x  + 
      \int_{\partial T \cap \partial \Omega} \sigma u_R v \, \mathrm{d}s \right).
\]

Koncept lokálních matic $\tn A_e$ a vektorů pravé strany $\vc b_e$ spočívá v tom, 
...
\[
b_T^i = \int_{T} f \phi_i \, \mathrm{d}x + 
...
Pro demonstraci si představme dva sousedící elementy $K$ a $L$, které sdílejí společnou hranu s vrcholy 
$N_i$, $N_j$. Jejich lokální matice $\tn A_K$ a $\tn A_L$ přispívají k celkové globální matici 
$\tn A$ tak, že prvky odpovídající společným uzlům se sčítají. Například pro řádeky a sloupce $i$ a $j$ 
sdílené oběma elementy přispívají ke globální matici oba elementy $K$ a $L$, ale případně i další elementy
s vrcholy $N_i$ a $N_j$:
\[
A^{ii} = A_K^{ii} + A_L^{ii} + \dots, \quad  A^{jj} = A_K^{jj} + A_L^{jj} + \dots, \quad
A^{ij} = A^{ji} = A_K^{ij} + A_L^{ij} + \dots.
\]

Tímto způsobem sestavíme globální matici $\tn A$ z lokálních matic
$A_K$ pro $K \in \elements$ je součtem všech lokálních příspěvků z jednotlivých elementů.
...
každý element $K$ zobrazíme pomocí {\it affinní transformace} na {\it referenční element} $\hat K$,
...
a splňují podmínku $\hat{\phi}_i(\hat{N}_j) = \delta_{ij}$. 
Jelikož josu bázové funkce lineární, jsou jejich gradienty konstantní:
...
$\hat{x} = (\hat{x}_1, \hat{x}_2)$ a souřadnice ve skutečném elementu $x = (x_1, x_2)$, 
pak afinní transformace má tvar:

\[
x = \tn F_T(\hat{x}) = \tn J_T \hat{x} + \vc j_T,
\]
...
Protože vrcholy referenčního trojúhelníka jsou definovány jako:
...
kde $\vc e_1$ a $\vc e_2$ jsou hrany skutečného trojúhelníka $T$ vycházející z vrcholu $\vc N_1$.
...
pomocí transformace gradientů:

\[
\nabla_x \phi_i(x) = \tn J_T^{-T} 
...
\begin{equation}
    \label{eq:jac_mat}
    \frac{\prtl x_i}{\prtl \hat{x}_j} = J_{T, i, j} 
\end{equation}
...
\[
    \grad_{\vc x} \phi_k = \tn J_T^{-T} \grad_{\hat{\vc x}} \hat\phi_k.
\]
...
\[
b_T^i = \int_{\hat{T}} f(\tn F_T(\hat{\vc x})) \hat{\phi}_i(\hat{\vc x}) 
|\det \tn J_T| \d\hat{\vc x} + 
\int_{\partial \hat{T} \cap \partial \hat{\Omega}} 
\sigma u_R(\tn F_T(\hat{\vc x})) \hat{\phi}_i(\hat{\vc x}) |\tn J_T \hat{\vc n}| \d\hat{s}.
\]

Zde $|\tn J_T \hat{\vc n}|$ představuje měřítko délky při transformaci hran.
