...
\begin{subequations}
\label{eq:ref_base_gradients}
\begin{align}
%\label{eq:ref_base_gradients1}
...
f(\vc F(\hat{\vc x})) 
\big|\frac{\d F}{\d \hat{\vc x}}\big| \d\hat{\vc x}.
\]
Po substituci se nám v integrálu objevil navíc tzv. {\it Jakobián}, 
tj.  absolutní hodnota determinantu derivace zobrazení $\bf F$.
Speciálně $\vc F = \vc F_T$ je afinní (lineární + konstanta) zobrazení, 
derivaci snadno spočteme z jeho definice
\eqref{eq:affine}:
\[
 \frac{\d \vc F_T}{\d\hat{\vc x}} = \tn J_T 
\]

Věta o substituci nám tak umožňuje převést integrály přes jednotlivé elementy 
na integrály přes referenční element, což značně usnadňuje jejich vyhodnocení.
Pro integrál přes element $T$ pak substitucí dostaneme konkrétnější tvar:
\[
...
Poslední ingediencí pro vyhodnocení integrálů na referenčním elementu je 
výpočet gradientů bázových funkcí. Z jejich definice totiž přímo dostaneme pouze 
gradienty \eqref{eq:ref_base_gradients} podle referenčních souřadnic $\hat{\vc x}$,
my však potřebujeme gradienty podle absolutních souřadnic $\vc x$.

Víme, že \(\phi_i(\vc x) = \hat{\phi}_i(\hat{\vc x})\), 
kde $\vc x$ závisí na $\hat{\vc x}$ skrze inverzní afinní transformaci $\vc F$.
Derivujme tedy $\hat\phi_(\hat{\vc x})$ pomocí řetízkového pravidla 
pro derivování složených funkcí více proměnných:
\begin{equation}
\label{eq:chain}
...
= \prtl_{\hat{x}_j} \vc F^\top \cdot \grad_{\vc x} \phi_k(\vc x) 
\end{equation}
V posledním členu je $\vc F$ (a tedy i jeho derivace) sloupcový vektor, proto musíme provést jeho 
transpozici, pro koretní zápis skalárního součinu. Aplikací pro $j=1, 2$ dostaneme
stejný výpočet ve vektorové formě:
\[
 \grad_{\hat{\vc x}} \hat\phi_k = \grad \vc F^\top \grad_{\vc x} \phi_k 
 = \tn J_T^\top \grad_{\vc x} \phi_k
\]
Násobením inverzní transponovenou Jacobiho maticí pak dostaneme
gradienty bázových funkcí vůči reálným souřadnicím $\vc x$:
\[
...
Použijeme-li výše uvedené vztahy, můžeme vyjádřit prvky lokální matice 
$\tn A_T$ a vektoru $\vc b_T$ na elementu $T$ 
jako integrály přes referenční element $\hat T$:

\begin{equation}
A_{T,ij} = \int_{\hat{T}} \left( \tn K \tn J_T^{-\top} \nabla_{\hat{\vc x}} \hat{\phi}_j(\hat{\vc x}) \right) 
         \cdot \left( \tn J_T^{-\top} \nabla_{\hat{\vc x}} \hat{\phi}_i(\hat{\vc x}) \right) |\tn J_T| 
         \d\hat{\vc x} 
...
\begin{equation}
b_{T,i} = \int_{\hat{T}} f \hat{\phi}_i(\hat{\vc x}) 
|\tn J_T| \d\hat{\vc x} + 
\int_{\partial \hat{T} \cap \partial \hat{\Omega}} 
\sigma_R u_R \hat{\phi}_i(\hat{\vc x}) |\tn J_T \hat{\vc n}| \d\hat{s}.
\end{equation}

Zde $|\tn J_T \hat{\vc n}|$ představuje měřítko délky při transformaci stran.
Zadané parametry $\tn K$, $\sigma$, $\sigma_R$, $u_R$ jsou obecně funkce 
proměnné absolutních souřadnic $\vc x$ ovšem často uvažované 
jako konstanty na jednotlivých elementech. V tom případě už můžeme vyhodnotit 
první integrál jelikož i gradienty i Jakobián jsou konstantní pro daný element $T$.
Pro ostatní integrály a obecné $\tn K$ však potřebujeme vhodnou 
aproximaci integrálů.
...
Definice Reimanova integrálu je založena na approximaci funkce pomocí po částech konstantní funkce na nějakém dělení integrované oblasti.
Konkrétně na obrázku \ref{fig:quad} vlevo je hodně hrubé dělení referenčního elementu a integrál bychom mohli 
approximovat součet:
\[
  \int_{\hat T} f(\vc x) \d x \approx f(\vc x_1) |T_1| + f(\vc x_2)|T_2| + f(\vc x_3)|T_3| = \sum_{i=1}^3 f(\vc x_i)w_i.
\]
Obecně můžeme integrál approximovat pomocí kvadratury, která je dána
\begin{enumerate}
 \item množinou kvadraturních bodů $Q=\{\vc q_i \in \hat T | i=1,\dots, n\}$,
 \item množinou jim příslušejících vah $w_q$.
\end{enumerate}
...
Riemannova definice používá limitu pro přes libovolné dělení, proto na přesnosti pro hrubá dělení nezáleží.
Pro dosažení dobré aproximace s minimem bodů bychom chtěli najít takové body $\vc q_i$ a jejich váhy $w_i$,
aby chyba vůči přesnému integrálu byla co nejmenší. Toho nelze dosáhnout pro obecnou funkci, ale lze toho dosáhnout 
pro pěkné hladké funkce, jako například pro polynomy. Konkrétně pro každé liché $p$ existuje tzv. Gaussova 
kvadratura, která integruje přesně polynomy až do řádu $p$ včetně. Na obrázku 
\ref{fig:quad} vpravo jsou pozice bodů pro $p=3$, Gausovy kvadratury pro $p=1, 3, 5$ jsou na obr. \ref{fig:gauss}

\begin{figure}
 \begin{center}
 \includegraphics[width=\textwidth]{img/quadrature.pdf}
 \caption{Riemannova integrace (vlevo) jako motivace pro Gaussovu kvadraturu (vpravo).}
 \label{fig:quad}
 \end{center}
 \end{figure}
 ...
 \begin{figure}
 \begin{center}
 \includegraphics[width=\textwidth]{img/gauss_135.pdf}
 \caption{Gaussova kvadratura pro polynomy do řádu $p=1,2,3$.}
 \label{fig:gauss}
 \end{center}
 \end{figure}

Jaký řád kvadratury tedy potřebujeme pro integrály v bilineární formě \eqref{eq:a_h_2D}? Pokud je $\tn K$ a $\sigma$ 
konstantní na elementu, pak v prvním integrálu máme polynom řádu $0$ a stačí kvadratura $p=1$. Ve druhém integrálu je součin
lineárních bázových funkcí což je polynom řádu $2$ a kvadratura musí být alespoň $p=3$. Pro nekonstantní koeficienty a
konečné prvky / bázové funkce vyššího řádu může být potřeba použít kvadratury vyšších řádů. 
...
\section{Praktické aspekty MKP}
\begin{itemize}
 \item Vyhodnocení forem založené na bázových funkcích tabulovaných v kvadraturních bodech (FENICS, DEAL.II).
 \item Assemblace globální matice a pravé strany z lokálních příspěvků.
 \item Obecná formulace konečného prvku, unisolventnost.
 \item Prvky 3D a prvky vyšších řádů.
\end{itemize}

Rozšiřující témata.
\begin{itemize}
 \item Integrály přes hranici.
 \item Příklady dalších rovnic.
 \item Evoluční a nelineární rovnice ...
 \item (ne - součást přednášky o aproximativních vlastnostech) Předpoklady na triangulaci, adaptivita.  
\end{itemize}
