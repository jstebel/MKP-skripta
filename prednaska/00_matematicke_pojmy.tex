\appendix
\section{Základy lineární algebry}
...
Neprázdná množina $V$, na které je definováno sčítání $+:V\times V\to V$ a násobení reálným číslem $\cdot:\R\times V\to V$, se nazývá {\bf reálný vektorový prostor} (nebo také lineární prostor), jestliže pro každé $\vec x, \yy, \zz\in V$ a každé $\alpha,\beta\in\R$ platí:
\begin{enumerate}
\item $\forall\vec x,\yy\in V:~\vec x+\yy=\yy+\vec x$ \hspace{\stretch{1}}{\footnotesize(komutativita sčítání)}
\item $\forall\vec x,\yy,\zz\in V:~(\vec x+\yy)+\zz=\vec x+(\yy+\zz)$ \hspace{\stretch{1}}{\footnotesize(asociativita sčítání)}
\item $\forall\alpha,\beta\in\R~\forall\vec x\in V:~\alpha\cdot(\beta\cdot\vec x)=(\alpha\beta)\cdot\vec x$ \hspace{\stretch{1}}{\footnotesize(asociativita násobení)}
\item $\forall\alpha\in\R~\forall\vec x,\yy\in V:~\alpha\cdot(\vec x+\yy)=\alpha\cdot\vec x+\alpha\cdot\yy$ \hspace{\stretch{1}}{\footnotesize(distributivita sčítání prvků z $V$)}
\item $\forall\alpha,\beta\in\R~\forall\vec x\in V:~(\alpha+\beta)\cdot\vec x=\alpha\cdot\vec x+\beta\cdot\vec x$ \hspace{\stretch{1}}{\footnotesize(distributivita sčítání čísel)}
\item $\forall\vec x\in\R:~1\cdot\vec x=\vec x$ \hspace{\stretch{1}}{\footnotesize(vlastnost reálného čísla $1$)}
\item $\exists\0\in V ~\forall\vec x\in V:~0\cdot\vec x=\0$ \hspace{\stretch{1}}{\footnotesize(existence nulového prvku)}
\end{enumerate}
\end{df}
...
\begin{itemize}
\item $\forall\vec x\in V$: $\vec x+\0=\vec x$,
\item $\forall\alpha\in\R$: $\alpha\cdot\0=\0$,
\item $\forall\vec x\in V~\forall\alpha\in\R,\alpha\neq 0$: $\alpha\cdot\vec x=\0\Rightarrow\vec x=\0$.
\end{itemize}
% 
% 
...
\begin{itemize}
\item[(i)] pro každé $\vec x,\yy\in W$ je $\vec x+\yy\in W$,
\item[(ii)] pro každé $\vec x\in W$ a $\alpha\in\R$ je $\alpha\vec x\in W$.
\end{itemize}
\end{veta}
...
\begin{df}
Nechť $\vec x_1,\vec x_2,\ldots,\vec x_n$ jsou prvky vektorového prostoru $V$ a $\alpha_1,\alpha_2,\ldots,\alpha_n$ jsou reálná čísla.
Vektor
$$ \alpha_1\vec x_1+\alpha_2\vec x_2+\ldots+\alpha_n\vec x_n $$
se nazývá {\bf lineární kombinací} vektorů $\vec x_1,\vec x_2,\ldots,\vec x_n$.
Číslům $\alpha_1,\alpha_2,\ldots,\alpha_n$ říkáme {\bf koeficienty} lineární kombinace.
\end{df}

\begin{df}
{\bf Triviální} lineární kombinace vektorů $\vec x_1,\vec x_2,\ldots,\vec x_n$ je taková lineární kombinace, která má všechny koeficienty nulové.
...
\begin{df}
Konečnou množinu vektorů $\{\vec x_1,\ldots,\vec x_n\}$ nazýváme {\bf lineárně závislou}, pokud existuje netriviální lineární kombinace těchto vektorů, která je rovna nulovému vektoru. Stručně říkáme, že vektory $\vec x_1,\ldots,\vec x_n$ jsou lineárně závislé.

Množina vektorů $\{\vec x_1,\ldots,\vec x_n\}$ se nazývá {\bf lineárně nezávislá}, pokud není lineárně závislá.
\end{df}
...
\begin{df}
{\bf Lineární obal} konečné množiny $\{\vec x_1,\ldots,\vec x_n\}$ je množina všech lineárních kombinací těchto vektorů.
Lineární obal nekonečné množiny $M$ je sjednocení lineárních obalů všech konečných podmnožin množiny $M$.
\end{df}

Lineární obal množiny $\{\vec x_1,\ldots,\vec x_n\}$ značíme $\lo{\vec x_1,\ldots,\vec x_n}$.
...
% \begin{veta}
% Nechť $\vec x_1,\ldots,\vec x_n$ jsou prvky vektorového prostoru $V$, z nichž alespoň jeden je nenulový.
% Pak z nich lze vybrat prvky $\vec x_{s_1}, \vec x_{s_2},\ldots,\vec x_{s_k}$, kde $1\le s_1<s_2<\ldots<s_k\le n$, takové, že
% \begin{itemize}
% \item[(i)] $\vec x_{s_1},\ldots,\vec x_{s_k}$ jsou lineárně nezávislé,
% \item[(ii)] $\lo{\vec x_{s_1},\ldots,\vec x_{s_k}}=\lo{\vec x_1,\ldots,\vec x_n}$.
% \end{itemize}
% \end{veta}

\begin{veta}
Nechť vektory $\vec x_1,\ldots,\vec x_n$ tvoří bázi vektorového prostoru $V$.
Je-li $\vec x\in V$ libovolný vektor, pak existuje právě jedna uspořádaná $n$-tice čísel $(c_1,\ldots,c_n)$ taková, že
$$ \vec x = c_1\vec x_1+\ldots+c_n\vec x_n. $$
\end{veta}

\begin{df}
Čísla $c_1,\ldots,c_n$ z předchozí věty se nazývají {\bf souřadnice} vektoru $\vec x$ v bázi $\vec x_1,\ldots,\vec x_n$.
\end{df}
...
\begin{veta}
Nechť $V$ je vektorový prostor, jehož dimenze je $\dim V=n$ a $M=\{\vec x_1,\ldots,\vec x_m\}$.
Pak platí:
\begin{enumerate}
...
\begin{df}
Zobrazení $f$ vektorového prostoru $V$ do vektorového prostoru $W$ se nazývá {\bf lineární}, jestliže pro každé $\alpha\in\R$ a $\vec x,\yy\in V$ platí:
$$ f(\alpha\vec x+\yy)=\alpha f(\vec x) + f(\yy). $$
...
\begin{veta}
Jsou-li vektory $\vec x_1,\ldots,\vec x_k$ lineárně závislé, pak jsou lineárně závislé také vektory $f(\vec x_1),\ldots,f(\vec x_k)$.
Pokud je navíc $f$ prosté, pak platí i obrácené tvrzení, tedy
$$ \vec x_1,\ldots,\vec x_k \mbox{ jsou lineárně závislé }\Leftrightarrow f(\vec x_1),\ldots,f(\vec x_k)\mbox{ jsou lineárně závislé}. $$
\end{veta}


\begin{df}
{\bf (Reálná) matice} řádu $m \times n$ je symbol
...
\end{df}
Množinu všech matic řádu $m\times n$ budeme značit $\R^{m\times n}$.
...
V dalším ztotožníme vektory z $\R^n$ s maticemi typu $(n,1)$, tj. $\vc a\in\R^n$ znamená totéž jako $\vc a\in\R^{n\times 1}$.

\begin{df}
Nechť $\A\in\R^{m\times n}$, $\xx=\begin{pmatrix}x_1\\\vdots\\x_n\end{pmatrix}\in\R^{n}$ a $\vc b=\begin{pmatrix}b_1\\\vdots\\b_m\end{pmatrix}\in\R^{m}$.
Pak maticovou rovnost
$$ \A\xx=\vc b $$
nazýváme {\bf soustavou $m$ lineárních rovnic o $n$ neznámých}.
Matici $\A$ nazýváme {\bf maticí soustavy} a vektor $\vc b$ nazýváme {\bf vektorem pravých stran}.
Připíšeme-li k matici soustavy do dalšího sloupce vektor $\vc b$ (pro přehlednost oddělený svislou čarou), dostáváme matici $(\A|\vc b)\in\R^{m\times(n+1)}$, kterou nazýváme {\bf rozšířenou maticí soustavy}.
\end{df}

  
\begin{df}
{\bf Řešením} soustavy $\A\xx=\vc b$ je takový vektor $\vc a=(\alpha_1,\ldots,\alpha_n)\in\R^n$, pro který platí: Dosadíme-li hodnoty $\alpha_i$ za symboly $x_i$, pak je splněna požadovaná maticová rovnost, tj.
$$ \A \begin{pmatrix}\alpha_1\\\vdots\\\alpha_n\end{pmatrix}=\begin{pmatrix}b_1\\\vdots\\b_m\end{pmatrix}. $$
{\bf Řešit soustavu} $\A\xx=\vc b$ znamená nalézt všechna její řešení, tj. nalézt podmnožinu $\R^n$ všech řešení této soustavy.
\end{df}

\begin{veta}[Frobeniova]
Soustava $\A\xx=\vc b$ má řešení právě tehdy, když
$$ \rank{\A}=\rank{\A|\vc b}, $$
...
\begin{df}
Nechť $\A\xx=\vc b$ je soustava $m$ lineárních rovnic o $n$ neznámých a $\mat C\xx=\vc d$ je soustava $k$ lineárních rovnic o stejném počtu $n$ neznámých.
Říkáme, že tyto soustavy jsou {\bf ekvivalentní}, pokud obě soustavy mají stejné množiny řešení.
\end{df}

\begin{veta}
Ke každé soustavě $\A\xx=\vc b$ lze nalézt ekvivalentní soustavu $\mat C\xx=\vc d$, jejíž matice $\mat C$ je horní trojúhelníková.
\end{veta}
  
\begin{df}
Existuje-li v matici $\vc b$ aspoň jeden prvek nenulový, říkáme, že soustava $\A\xx=\vc b$ je {\bf nehomogenní}.
Jsou-li všechny prvky v matici $\vc b$ nulové, nazýváme soustavu rovnic {\bf homogenní} a zapisujeme ji takto:
$$ \A\xx=\0. $$
\end{df}
...
\begin{df}
Libovolné řešení $\vc v\in\R^n$ nehomogenní soustavy lineárních rovnic $\A\xx=\bb$ o $n$ neznámých se nazývá {\bf partikulární řešení} této soustavy.
...
\begin{veta}
\begin{enumerate}
\item Nechť $\vc v$ je partikulární řešení nehomogenní soustavy $\A\xx=\bb$ a $\uu$ je libovolné řešení přidružené homogenní soustavy $\A\xx=\0$.
Pak $\vc v+\uu$ je také řešením soustavy $\A\xx=\bb$.
\item Nechť $\vc v$ a $\vc w$ jsou dvě partikulární řešení nehomogenní soustavy $\A\xx=\bb$.
Pak $\vc v-\vc w$ je řešením přidružené homogenní soustavy $\A\xx=\0$.
\end{enumerate}
\end{veta}
  
\begin{veta}
Nechť $\vc v$ je partikulární řešení soustavy $\A\xx=\bb$ a $M_0$ je vektorový prostor všech řešení přidružené homogenní soustavy $\A\xx=\0$.
...
\section{Diferenciální počet funkce více proměnných}
