...
Je dána oblast $\Omega\subset\R^d$ s hranicí $\partial\Omega=\overline\Gamma_n\cup\overline\Gamma_r$, vektorové pole $\vc v:\Omega\to\R^d$ a skalární funkce $\sigma_r$, $u_r$ na $\Gamma_r$.
Hledáme funkci $u:\overline\Omega\to\R$, která splňuje:
\[ \begin{aligned}
-\Delta u + \div(\vc v u) &= 0 &&\text{v }\Omega,\\
-\grad u \cdot \vc n &= 0 &&\text{na }\Gamma_n,\\
(-\grad u + \vc v u) \cdot \vc n &= \sigma_r(u - u_r) &&\text{na }\Gamma_r.
\end{aligned} \]
Slabá formulace:
\[ \mbox{Najdi }u\in V:=H^1(\Omega) \mbox{ takové, že }\forall v\in H^1(\Omega):~a(u,v)=l(v), \]
kde
\begin{equation}\label{eq:def_a_ad}
a(u,v) := (\grad u-\vc v u, \grad v) + (\vc v\cdot\vc n u,v)_{\Gamma_n} + (\sigma_r u, v)_{\Gamma_r},
\end{equation}
...
% 
\begin{veta}
Nechť $\vc v\in L^\infty(\Omega;\R^d)$, $\vc v\cdot\vc n\in L^\infty(\Gamma_n)$ a $\sigma_r\in L^\infty(\Gamma_r)$. Pak existuje konstanta $\beta:=\beta(\norm{\vc v}_\infty,\norm{\vc v\cdot\vc n}_{\infty,\Gamma_n},\norm{\sigma_r}_{\infty,\Gamma_r})$ taková, že
...
\begin{align*}
(\vc v v,\nabla w)
&\le \sum_{i=1}^d(|v_i||v|,|\partial_{x_i}w|)
\le \norm{\vc v}_\infty\sum_{i=1}^d(|v|,|\partial_{x_i}w|)\\
&\le \norm{\vc v}_\infty\norm{v}_2\sum_{i=1}^d\norm{\partial_{x_i}w}_2
\le d\norm{\vc v}_\infty\norm{v}_2\norm{\nabla w}_2\\
&\le d\norm{\vc v}_\infty\norm{v}_{1,2}\norm{w}_{1,2},
\end{align*}
Pro odhad hraničních členů použijeme navíc větu o stopách:
\[ (\vc v\cdot\vc n v,w)_{\Gamma_n} \le \norm{\vc v\cdot\vc n}_{\infty,\Gamma_n}\norm{v}_{2,\Gamma_n}\norm{w}_{2,\Gamma_n}
\le C_1^2\norm{\vc v\cdot\vc n}_{\infty,\Gamma_n}\norm{v}_{1,2}\norm{w}_{1,2}, \]
\begin{align*}
...
V souhrnu jsme dokázali, že
\[ a(v,w) \le \left(1+d\norm{\vc v}_\infty+C_1^2\norm{\vc v\cdot\vc n}_{\infty,\Gamma_n}+C_2^2\norm{\sigma_r}_{\infty,\Gamma_r}\right)\norm{v}_{1,2}\norm{w}_{1,2} =: \beta \norm{v}_{1,2}\norm{w}_{1,2}. \]
...
Nechť jsou splněny následující předpoklady:
\begin{itemize}
\item[(i)] $\vc v$ má nulovou divergenci, $\vc v\cdot\vc n\ge 0$ na $\Gamma_n$ a $\vc v\cdot\vc n\le 0$ na $\Gamma_r$;
...
\end{veta}
\begin{proof}
Jelikož $\div\vc v=0$ a $\vc v\cdot\vc n\le 0$ na $\Gamma_r$, platí:
\[ -(\vc v v,\nabla v) = -(\vc v,\nabla\frac{v^2}2) = -\frac12(\vc v\cdot\vc n v,v)_{\partial\Omega} +(\div\vc v,\frac{v^2}2) \ge -\frac12(\vc v\cdot\vc n v,v)_{\Gamma_n}, \]
a proto také
\begin{align*}
a(v,v) & = \norm{\nabla v}_2^2 - (\vc v v,\nabla v) + (\vc v\cdot\vc n v,v)_{\Gamma_n} + (\sigma_r v,v)_{\Gamma_r}\\
&\ge \norm{\nabla v}_2^2 + \frac12 (\vc v\cdot\vc n v,v)_{\Gamma_n} + (\sigma_r v,v)_{\Gamma_r}\\
