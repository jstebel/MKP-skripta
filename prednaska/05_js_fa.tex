...
Ve slabé formulaci různých úloh se vyskytují lineární a bilineární formy a prostory funkcí, v nichž hledáme řešení.
V této kapitole se budeme zabývat základními vlastnostmi prostorů, v nichž je vhodné hledat slabé řešení.
...
V následujících definicích tyto pojmy zobecníme a zavedeme pro libovolný vektorový prostor.
\begin{df}
Nechť $V$ je vektorový prostor.
\begin{itemize}
\item[(i)] Řekneme, že zobrazení $b:V\to\Real$ je \emph{lineární forma} na $V$, pokud pro každé $u,v\in V$ a~$\alpha,\beta\in\Real$ platí:
...
Na vektorovém prostoru může existovat více norem. Norma obecně nemusí být indukovaná skalárním součinem, musí však splňovat následující vlastnosti.
\begin{df}
...
Norma $\norm{\ }_\infty$ stanoví rozdíl dvou funkcí jako největší odchylku jejich hodnot.
Naopak norma $\norm{\ }_2$ měří plochu mezi grafy druhých mocnin těchto funkcí.
Přestože význam $\norm{\ }_2$ se může jevit méně intuitivní, má tato norma své opodstatnění. První vlastností je, že byla zavedena pomocí skalárního součinu (totéž nelze říci o $\norm{\cdot}_\infty$). Skalární součin hraje v úlohách spojených s MKP důležitou roli. 
...
Důležitou otázkou je, jak lze nalézt limitu posloupnosti (tj. prvek $u=\lim_{n\to\infty} u_n$).
Při vyšetřování konvergence posloupnosti \emph{funkcí} je vhodné nejprve zjistit, zda existuje tzv. \emph{bodová limita}.
Bodová limita je definována jako funkce $U$ splňující:
\[ \forall x: ~U(x) = \lim_{n\to\infty}u_n(x). \]
Všimněme si, že hodnota $U(x)$ je určena jako limita hodnot $u_n(x)$, tedy jedná se o klasickou limitu číselné posloupnosti.
Platí, že pokud posloupnost spojitých funkcí $\{u_n\}$ definovaných na omezené oblasti konverguje k nějaké funkci $u$ v normě $\norm{\ }_2$ nebo $\norm{\ }_\infty$, pak prvek $u$ se skoro všude shoduje s bodovou limitou $U$, tj.
\[ u(x) = (\lim_{n\to\infty} u_n)(x) = \lim_{n\to\infty} (u_n(x)) = U(x). \]
Teprve poté, co najdeme bodovou limitu, je možné ověřit, zda se jedná také o limitu v normě.
...
\end{ex}
% 
Uvedený příklad poukazuje na to, že existence limity v normovaném lineárním prostoru závisí na tom, jakou používáme normu.
...
% 
Skutečnost, že daná posloupnost v normovaném lineárním prostoru je konvergentní, závisí nejen na zvolené normě, ale také na prostoru samotném.
...
Pojem úplnost souvisí s tzv. cauchyovskými posloupnostmi.
% 
\begin{df}
...
\end{df}
% 
Nepřesně řečeno, vzdálenost prvků cauchyovské posloupnosti s rostoucími indexy klesá a může být libovolně malá.
Cauchyovská posloupnost obecně nemusí mít limitu, není-li prostor dostatečně \uv{bohatý}. Avšak každá konvergentní posloupnost je nutně cauchyovská.
% 
\begin{df}
...
Úplný prostor se skalárním součinem se pak nazývá \emph{Hilbertův prostor}.
\end{df}
% 
\begin{ex}
...
Místo spojitosti bude uvažována \emph{integrovatelnost} funkcí, čímž je míněno, že funkce má konečný integrál.

\begin{df}
\label{df:Lp}
Prostorem $L^2(\Omega)$ rozumíme množinu funkcí integrovatelných s první a druhou mocninou:
\[ L^2(\Omega):=\left\{u:\Omega\to\R;~u \mbox{ a } |u|^2 \mbox{ jsou integrovatelné} \right\}. \]
Spolu s normou $\norm{\ }_2$ tvoří $L^2(\Omega)$ normovaný lineární prostor.
\end{df}

Z definice plyne, že každá spojitá funkce v $\overline\Omega$ patří do prostoru $L^2(\Omega)$.
Do tohoto prostoru ovšem patří i mnoho dalších funkcí, které mohou být nespojité nebo neomezené.
...
% Požadavek na integrovatelnost funkce $u$ je splněn pro většinu \uv{kulturních} funkcí (např. pro spojité funkce).
% Existují však i nepěkné funkce, jejichž druhá mocnina je integrovatelná, zatímco funkce samotná ne.
...
\begin{veta}
Prostor $L^2(\Omega)$ s normou $\norm{\ }_2$ je nejmenší úplný normovaný lineární prostor obsahující všechny spojité funkce na $\overline\Omega$.
\end{veta}

Obdobně lze zavést také prostor
...
Pro spojité funkce tento vztah splývá s původní definicí $\norm{\ }_\infty$, tedy lze místo infima použít minimum a slova \uv{skoro všude} vypustit.
Také $L^\infty(\Omega)$ je úplný prostor.
\begin{ex}
...
Pojmy norma, skalární součin a prostor $L^2(\Omega)$ lze přímočaře zobecnit pro vektorové funkce.
Pro $\vc u,\vc v:\Omega\to\R^d$ definujeme
...
Proto zavedeme další prostor funkcí s integrovatelnými derivacemi.
...
\end{df}
\begin{veta}
Prostor $H^1(\Omega)$ s normou $\norm{\cdot}_{1,2}$ je úplný.
\end{veta}

% priklady nespojitych funkci v H1 (2d, 3d)
\begin{ex}
Na oblasti $\Omega=\{(x,y);~x^2+y^2=1\}\setminus\{(x,0);~x\in(-1,0)\}$ uvažujme funkci
...
Řekneme, že hranice oblasti $\Omega$ je Lipschitzovská, jestliže ji lze lokálně popsat jako graf spojité funkce s omezenými derivacemi.
...
Nyní zavedeme prostory funkcí s nulovou stopou, které hrají důležitou roli u slabých formulací s Dirichletovou podmínkou:
\begin{df}
Pro netriviální část $\Gamma$ hranice oblasti $\Omega$ definujeme
...
V této kapitole zavedeme jednotný koncept pro různé druhy slabých formulací, budeme se zabývat otázkou řešitelnosti a aproximace těchto úloh.

\subsection{Abstraktní variační úloha}

Nechť $V$ je úplný normovaný lineární prostor s normou $\norm{\ }$, $a$ a $b$ je bilineární, resp. lineární forma na $V$.
Úlohu
\begin{equation}
\label{eq:variational_problem}
\text{Najdi }u\in V \text{ takové, že }\forall v\in V:~a(u,v)=b(v),
\end{equation}
...
Nechť $V$ je úplný normovaný lineární prostor se skalárním součinem, $a$ je bilineární forma na $V$, pro niž existují konstanty $\alpha,\beta>0$ takové, že
\begin{itemize}
...
\[ \forall v\in V:~\alpha\norm{v}^2\le a(v,v), \]
\end{itemize}
a nechť $b$ je omezená lineární forma na $V$, tj.
\begin{itemize}
\item[(iii)] existuje konstanta $\gamma>0$ taková, že 
\[ \forall v\in V:~b(v)\le \gamma\norm{v}. \]
\end{itemize}
...
Pak $u$ bude splňovat Dirichletovu podmínku právě tehdy, když $u-u_d\in H^1_0(\Omega)$.
Řešení budeme hledat ve tvaru $u=z+u_d\in H^1(\Omega)$, kde $z\in H^1_0(\Omega)$ splňuje
\[ 
    \forall v\in H^1_0(\Omega):\ a(z,v)=b(v).
\]
Formy $a$ a $b$ jsou dány vztahy:
...
\begin{veta}
    ...
    \[ \forall v\in H^1(\Omega):~b(v) \le \gamma\norm{v}_{1,2}, \]
    kde lineární forma $b$ je definována v \eqref{eq:def_l_nonhom_dir}.
\end{veta}
\begin{proof}
...
Jsou tedy splněny předpoklady Lax-Milgramovy věty a úloha má právě jedno slabé řešení.



\subsubsection{Neumannova podmínka}

Hledáme funkci $u:\overline\Omega\to\R$, která splňuje:
\[ 
\begin{aligned}
    -\Delta u &= f &&\text{v }\Omega,\\
    \nabla u\cdot\vc n&=0 && \text{na }\partial\Omega.
\end{aligned} 
\]
Protože je předepsána Neumannova podmínka na $\partial\Omega$, slabé řešení budeme hledat v prostoru $V:=H^1(\Omega)$.
Slabé řešení $u\in V$ bude splňovat
\[ 
    \forall v\in V:\ a(u,v)=b(v),
\]
kde
\begin{equation}\label{eq:def_a_neu}
    a(u,v) := (\grad u, \grad v),
\end{equation}
\begin{equation}\label{eq:def_l_neu}
    b(v) := (f,v).
\end{equation}
Obdobně jako u úlohy s Dirichletovou podmínkou lze ukázat, že formy $a$ a $b$ jsou omezené na prostoru $V$.
Avšak $a$ není na $V$ eliptická! To lze ukázat protipříkladem, zvolíme-li např. $v(x)=1$.
Tato funkce patří do $V$, má nenulovou normu $\norm{v}_{1,2} = |\Omega|^{1/2}$ a platí:
\[ a(v,v) = 0. \]
Proto podmínka elipticity nemůže být splněna s žádnou konstantou $\alpha>0$.

Platí také, že úloha s Neumannovou podmínkou na celé hranici nemá jednoznačné řešení --- pokud k nějakému řešení přičteme libovolnou konstantní funkci, získáme další řešení.

Pro jednoznačné určení řešení je třeba k úloze přidat dodatečnou podmínku, např. na střední hodnotu:
\[ \int_\Omega u = 0. \]
Pak bude prostorem pro variační úlohu množina $V_0:=\{v\in H^1(\Omega);~\int_\Omega v=0\}$.

\begin{veta}
    Bilineární forma $a$ je eliptická na $V_0$.
\end{veta}
Jsou tedy splněny předpoklady Lax-Milgramovy věty a úloha má právě jedno slabé řešení.
...
\section{Abstraktní odhad chyby}

Metoda konečných prvků pro řešení variační úlohy typu \eqref{eq:variational_problem} se nazývá Galerkinova metoda.
Její myšlenkou je nahradit prostor funkcí $V$ vhodným podprostorem $V_h$ konečné dimenze s vhodnou bází (typicky se jedná o podprostory funkcí po částech polynomiálních).
Výslednou úlohu je pak možné převést na soustavu lineárních rovnic.
Řešení variační úlohy na podprostoru $V_h$ je aproximací slabého řešení a jeho odchylku lze kvantifikovat v závislosti na volbě podprostoru.



% JB: následující je již v sekci 4.1; TODO: sloučit výklad
% JS: Je to podobne, ale neni to totez. V kap. 4 se resi konkretni problem, zde je abstraktni - pro jakoukoliv bilinearni a linearni formu.

\subsection{Aproximace abstraktní variační úlohy}
...
\mbox{Najdi }u_h\in V_h:~\forall v_h\in V_h~a(u_h,v_h)=b(v_h).
\end{equation}
...
\[ u_h=\sum_{j=1}^N u_j\varphi_j,~\vc u=(u_1,...,u_N)^\top\in\R^N. \]
...
Vzniklá soustava $N$ rovnic je lineární a neznámými jsou koeficienty $u_1,...,u_N$.
Můžeme ji přepsat do maticového tvaru:
\[ \tn A\vc u := \begin{pmatrix}a(\varphi_1,\varphi_1) & ... & a(\varphi_N,\varphi_1)\\&...&\\a(\varphi_1,\varphi_N) & ... & a(\varphi_N,\varphi_N)\end{pmatrix}\begin{pmatrix}u_1\\\vdots\\ u_N\end{pmatrix} = \begin{pmatrix}b(\varphi_1)\\\vdots\\b(\varphi_N)\end{pmatrix} =: \vc b, \]
% kde
% \[ \tn A = \begin{pmatrix}a(\varphi_1,\varphi_1) & ... & a(\varphi_N,\varphi_1)\\&...&\\a(\varphi_1,\varphi_N) & ... & a(\varphi_N,\varphi_N)\end{pmatrix}, \]
kde prvek $a_{ij}$ matice $\tn A$ je roven $a(\varphi_j,\varphi_i)$ a prvek $b_i$ vektoru $\vc b$ je roven $b(\varphi_i)$, $i,j=1,...,N$.

Lze snadno ověřit, že z vlastností formy $a$ vyplývají obdobné vlastnosti matice $\tn A$: Např. je-li $a$ symetrická, pak také $\tn A$ je symetrická; je-li $a$ eliptická, pak $\tn A$ je pozitivně definitní, a tedy regulární, a soustava $\tn A\vc u=\vc b$ má právě jedno řešení.


\subsection{Abstraktní odhad chyby}
...
Nechť $w_h\in V_h$. Protože $V_h$ je podprostor $V$, je také $w_h\in V$ a platí $a(u,w_h) = b(w_h)$ a $a(u_h,w_h)=b(w_h)$.
