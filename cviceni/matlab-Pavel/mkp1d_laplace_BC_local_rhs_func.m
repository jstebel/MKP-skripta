function [x,u,e] = mkp1d_laplace_BC_local_rhs_func(L,n)
...
exact = @(x) 1/K * (0.25*sin(2*x)- C1*x - C2);
...
%%%%% Vypocet %%%%%
  ...
  for elem = 1:n
    ...
    % pouziti num. kvadratury na intervalu [x(elem), x(elem+1)]
    % pro integrand f * phi'(x), kde phi'(x)=+-1/h
    nq = 3;
    [xx, ww] = gauss_1d(nq,x(elem),x(elem+1));
    m = zeros(nM,1);
    for q = 1:nq
      m(1) = m(1) + f(xx(q))* (-1/h*(xx(q) - x(elem+1))) * ww(q);
      m(2) = m(2) + f(xx(q))* (1/h*(xx(q) - x(elem))) * ww(q); % f(xx(q))*
    end
