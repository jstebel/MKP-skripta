function [x,u,e] = mkp1d_5_func(startPoint,endPoint,n)
...
C1 = (sigma*(uB+d*L-uD) + d*(startPoint+1)) / (1-sigma*log((1+startPoint)/(1+endPoint)));
C2 = uB + d*endPoint - C1*log(1+endPoint);
exact = @(x) [C2 - d*x + C1*log(1+x);       -d + C1./(1+x)];
