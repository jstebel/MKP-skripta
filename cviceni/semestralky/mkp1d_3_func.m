function [x,u,e] = mkp1d_3_func(startPoint,endPoint,n)
...
tempvar = d/(K*K+c*c)
C2 = qN/c - K*tempvar;
C1 = -C2*exp(c*endPoint/K) - tempvar*(c*sin(endPoint) + K*cos(endPoint));
exact = @(x) [C1 + C2*exp(c/K*x) + tempvar*(c*sin(x)+K*cos(x));        C2*c/K*exp(c/K*x) + tempvar*(c*cos(x)-K*sin(x))];
