function [x,u,e] = mkp1d_6_func(startPoint,endPoint,n)
...
C1 = qN + d/2*(endPoint*endPoint + c); 
C2 = uA - d*startPoint/2 + C1/sqrt(c)*atan(startPoint/sqrt(c));
exact = @(x) [d/2*x - C1/sqrt(c)*atan(x/sqrt(c)) + C2;       d/2 - C1./(x.*x+c)];
